...
(function() {
   ...
   */
  function initialize() {
    console.log('[ContentExtension.validation] Initializing validation and testing');
  ...
  function checkLibraryAvailability() {
    ...
    console.log('  validateAccessibilityLibraries:', typeof window.validateAccessibilityLibraries);
    console.log('  NEXUS_TESTING_MODE:', window.NEXUS_TESTING_MODE);

    if (window.DOMAccessibilityAPI) {
      console.log('  DOMAccessibilityAPI functions:', Object.keys(window.DOMAccessibilityAPI));
    }
    if (window.AriaQuery) {
  ...
  function validateAccessibilityLibraries(el) {
    ...
    try {
      if (window.DOMAccessibilityAPI) {
        ...
        results.libraryResults.name = window.DOMAccessibilityAPI.computeAccessibleName(el);
        results.libraryResults.description = window.DOMAccessibilityAPI.computeAccessibleDescription(el);
      ...
      if (window.AriaQuery) {
        ...
        console.log('[VALIDATION] ARIA Query role:', results.libraryResults.role_aria);
      } else {
    ...
    try {
      if (CE.accessibility) {
        results.fallbackResults.name = CE.accessibility.computeFallbackAccessibleName(el);
        results.fallbackResults.description = CE.accessibility.computeFallbackDescription(el);
  ...
  function compareValidationResults(results) {
    // Compare names
    if (results.libraryResults.name !== undefined && 
    ...
    if (results.libraryResults.description !== undefined && 
        results.fallbackResults.description !== undefined) {
      results.comparison.descriptionMatch = 
        results.libraryResults.description === results.fallbackResults.description;
    ...
    // Compare roles
    const libraryRole = results.libraryResults.role_dom || results.libraryResults.role_aria;
    if (libraryRole && results.fallbackResults.role) {
      results.comparison.roleMatch = libraryRole === results.fallbackResults.role;
  ...
  function batchValidateAccessibility(selector = '*', limit = 10) {
    ...
    console.log(`[BATCH VALIDATION] Testing ${Math.min(elements.length, limit)} elements matching: ${selector}`);
  ...
  function injectLibrariesIntoPageContext() {
    try {
      ...
      domApiScript.src = chrome.runtime.getURL('src/libs/dom-accessibility-api.js');
      domApiScript.onload = () => {
      ...
      console.error('[NEXUS] Failed to inject DOM API into page context:', error);
    }
  }
  ...
  function injectAriaQuery() {
    try {
      ...
      console.error('[NEXUS] Failed to inject ARIA Query into page context:', error);
    }
  }
  ...
  function injectValidationFunctions() {
    try {
      ...
      validationScript.src = chrome.runtime.getURL('src/libs/validation-functions.js');
  ...
  async function testCurrentElement() {
    ...
    try {
      ...
      if (CE.accessibility && CE.accessibility.getAccessibleInfo) {
        const accessibilityInfo = await CE.accessibility.getAccessibleInfo(element, true);
  ...
  function runValidationSuite(options = {}) {
    ...
    try {
      results.tests.batchValidation = batchValidateAccessibility(batchSelector, batchLimit);
