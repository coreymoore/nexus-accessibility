...
window.ContentExtension.deps.defineModule(
  ...
  function (deps, utils) {
    ...
    // Ensure our namespace exists for backward compatibility
    ...
    // Note: Global element storage now managed by content-state-manager.js
    // window.nexusTargetElement is maintained by the state manager for CDP compatibility
    ...
    function storeElementForCDP(element, selectionType = "focus") {
      ...
      // Store the element globally for CDP access (direct approach)
      window.nexusTargetElement = element;

      // Also try to update the state manager if available
      try {
        const moduleRegistry = window.ContentExtension?.deps?.moduleRegistry;
        if (moduleRegistry) {
          const stateManagerModule = moduleRegistry.modules.get("stateManager");
          if (stateManagerModule?.exports) {
            stateManagerModule.exports.updateInspectedElement(element, {
              source: `cdp-${selectionType}`,
            });
          }
        }
      } catch (error) {
        console.warn(
          "Failed to update state manager from storeElementForCDP:",
          error
        );
      }
    ...
    // Public API combining all utilities
    const api = {
      ...
      // Module method calling helper
      callModuleMethod: function (moduleName, methodName, ...args) {
        const registry = window.ContentExtension?.deps;
        if (registry && registry.getModule) {
    ...
    console.log("[ContentExtension.utils] Module loaded (dependency system)");
    return api;
  }
);
