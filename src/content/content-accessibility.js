...
window.ContentExtension.deps.defineModule(
  ...
  function (deps, utils) {
     ...
     * @param {Element} element - The actual DOM element (needed for summary elements)
     * @returns {boolean|null} Normalized expanded state
     */
    function normalizeExpandedState(states, ariaProperties, element) {
      // Special case for summary elements: check parent details element
      if (element && element.tagName.toLowerCase() === "summary") {
        ...
        if (
          ...
          console.log("[NEXUS] Summary element expanded state check:", {
            elementTag: element.tagName,
            parentTag: parentDetails.tagName,
            detailsHasOpen: isOpen,
            detailsOpenAttr: parentDetails.getAttribute("open"),
            detailsOpenProp: parentDetails.open,
          });
          return isOpen;
        }
      }
    ...
    async function waitForAccessibilityUpdate(
      target,
      maxAttempts = window.NexusConstants?.RETRY_ATTEMPTS
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          ...
          if (attempt < maxAttempts - 1) {
            const delay =
              (window.NexusConstants?.TIMEOUTS?.ACCESSIBILITY_RETRY_BASE ||
                50) *
              (attempt + 1);
            console.log(
    ...
    async function getAccessibleInfo(target, forceUpdate = false) {
      // Skip inspector elements to prevent self-inspection
      if (
        target &&
        target.closest &&
        target.closest(".nexus-accessibility-ui-inspector")
      ) {
        console.log(
          "[ContentExtension.accessibility] Skipping inspector element, checking for focused/inspected elements"
        );

        // Get focus state from state manager instead of events module
        const focusState =
          utils.callModuleMethod("stateManager", "getCurrentState") || {};
        const { lastFocusedElement, inspectedElement } = focusState;
        const fallbackElement = inspectedElement || lastFocusedElement;

        if (fallbackElement && fallbackElement !== target) {
          console.log(
            "[ContentExtension.accessibility] Using fallback element:",
            fallbackElement.tagName,
            fallbackElement.id
          );
          // Recursively call with the fallback element
          return getAccessibleInfo(fallbackElement, forceUpdate);
        } else {
          console.log(
            "[ContentExtension.accessibility] No valid fallback element available, returning null"
          );
          return null;
        }
      }
      ...
      const promise = (async () => {
        ...
        if (!forceUpdate && cache) {
          ...
          if (cached) {
            console.log(
              "Using cached accessibility data (selective cache bypass):",
              {
                role: cached.role,
                name: cached.name,
            ...
            // Show inspector with cached data
            ...
            if (!inspectorResult) {
              console.error(
                "[ContentExtension.accessibility] Failed to show inspector for cached data"
              );
            }
            return cached; // Early return - skip expensive CDP calls!
          }
        }

        console.log(
          "Cache miss or forceUpdate - fetching fresh accessibility data:",
          {
            forceUpdate: forceUpdate,
        ...
        if (CE.utils.shouldUseLocalFallback(target)) {
          ...
          if (!inspectorResult) {
            console.error(
              "[ContentExtension.accessibility] Failed to show inspector for local fallback"
            );
          }
          return localInfo;
        }
        ...
        try {
          ...
          if (info?.role === "IframePresentational" && CE.utils.isInIframe()) {
            ...
            if (!inspectorResult) {
              console.error(
                "[ContentExtension.accessibility] Failed to show inspector for iframe fallback"
              );
            }
            return localInfo;
          }
          ...
          // Show inspector with the accessibility information
          const inspectorResult = utils.callModuleMethod(
            "inspector",
            "showInspector",
            result,
          ...
          if (!inspectorResult) {
            console.error(
              "[ContentExtension.accessibility] Failed to show inspector"
            );
          }
          ...
          if (result?.role === "generic" && CE.utils.isInShadowRoot(target)) {
            ...
            if (
              ...
              if (!inspectorResult) {
                console.error(
                  "[ContentExtension.accessibility] Failed to show inspector for shadow DOM fallback"
                );
              }
              return localInfo;
            }
    ...
    function processAccessibilityInfo(info, target) {
      ...
      const normalizedExpanded = normalizeExpandedState(
        states,
        ariaProperties,
        target
      );
      ...
      // Debug logging for summary elements
      if (target && target.tagName.toLowerCase() === "summary") {
        console.log("[NEXUS] Summary element processing result:", {
          elementTag: target.tagName,
          parentTag: target.parentElement?.tagName,
          normalizedExpanded: normalizedExpanded,
          originalStates: info?.states,
          originalAriaProps: info?.ariaProperties,
        });
      }
    ...
    // Return the accessibility module API
    return {
      initialize,
      cleanup,
      onStateChange,
    ...
    return CE.accessibility;
  }
);

console.log("[ContentExtension.accessibility] Module defined");
