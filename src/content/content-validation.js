  ...
  function (deps, utils) {
    ...
    function validateAccessibilityLibraries(el) {
      ...
      if (window.ValidationCore) {
        ...
        results = window.ValidationUtils.validateAccessibilityLibrariesCore(
          el,
          {
            verbose: true,
            useLibraries: window.NEXUS_TESTING_MODE?.useLibraries !== false,
      ...
      try {
        const accessibilityAvailable = utils.callModuleMethod(
          "accessibility",
          "computeFallbackAccessibleName",
          el
        );
        if (accessibilityAvailable !== undefined) {
          results.fallbackResults.name = accessibilityAvailable;
          results.fallbackResults.description = utils.callModuleMethod(
            "accessibility",
            "computeFallbackDescription",
            el
          );
          results.fallbackResults.role = utils.callModuleMethod(
            "accessibility",
            "computeFallbackRole",
            el
          );
          console.log(
            "[VALIDATION] Fallback results:",
      ...
      const prodResult = utils.callModuleMethod(
        "accessibility",
    ...
    function injectLegacyValidationFunctions() {
      try {
        ...
        validationUtilsScript.onerror = (error) => {
          console.error(
            "[NEXUS] Failed to load legacy ValidationUtils:",
            error
          );
        };
    ...
    async function testCurrentElement() {
      // Get the focused element from state manager
      const focusState =
      ...
      try {
        ...
        const accessibilityInfo = await utils.callModuleMethod(
          "accessibility",
          "getAccessibleInfo",
          element,
          true
        ...
        if (accessibilityInfo) {
          console.log(
            "[VALIDATION] Live accessibility info:",
            accessibilityInfo
          );
    ...
    function cleanup() {
      console.log(
        "[ContentExtension.validation] Cleaning up validation module"
      );
